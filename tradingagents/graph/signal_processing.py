...
class SignalProcessor:
    ...
    def process_signal(self, full_signal: str, stock_symbol: str = None) -> dict:
        ...
        # 如果信号已经是 JSON 格式，尝试直接解析
        try:
            import json
            decision_data = json.loads(full_signal)
            if 'action' in decision_data and 'reasoning' in decision_data:
                logger.info(f"✅ [SignalProcessor] 检测到结构化 JSON，直接解析成功: {decision_data}")
                return {
        ...
        messages = [
            {
                "role": "system",
                "content": f"""您是一位专业的金融分析助手，负责从交易员的分析报告中提取结构化的投资决策信息。
        ...
        try:
            ...
            if json_match:
                ...
                try:
                    ...
                    def _fq(s):
                        r = []; ins = False; esc = False
                        for c in s:
                            if esc: r.append(c); esc = False; continue
                            if c == "\\": r.append(c); esc = True; continue
                            if c == '"': ins = not ins; r.append(c); continue
                            if not ins and c == "'": r.append('"'); continue
                            r.append(c)
                        return "".join(r)
                    fixed_text = _fq(fixed_text)
                    try:
                        decision_data = json.loads(fixed_text)
                    except json.JSONDecodeError:
                        logger.error("JSON repair failed, using default")
