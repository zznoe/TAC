...
class GraphSetup:
    ...
    def setup_graph(
        ...
        Args:
            ...
            checkpointer: Optional checkpointer for state persistence and resumption.
                Pass a MemorySaver instance or custom checkpointer (e.g., RedisCheckpointer).
        """
        ...
        join_analysts_node = create_analysts_join_node()
        ...
        # 为每个分析师设置工具循环和结束到 Join 节点的路径
        for analyst_type in selected_analysts:
            current_analyst = f"{analyst_type.capitalize()} Analyst"
            current_tools = f"tools_{analyst_type}"

            # 添加条件边：如果需要工具则去 tools，否则去 Join
            workflow.add_conditional_edges(
                current_analyst,
                getattr(self.conditional_logic, f"should_continue_{analyst_type}"),
                {
                    current_tools: current_tools,
                    "Join Analysts": "Join Analysts"
                },
            )
            # 工具执行完后回到分析师
