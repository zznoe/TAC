...
class IntegratedCacheManager:
    ...
    def _start_cache_cleanup_thread(self, interval_hours: int = 1):
        """启动后台定时缓存清理线程"""
        def cleanup_worker():
            while True:
                try:
                    time.sleep(interval_hours * 3600)
                    self.logger.info("🔄 [后台] 触发缓存清理任务...")
                    self.clear_expired_cache()
                    self.logger.info("✅ [后台] 缓存清理完成")
                except Exception as e:
                    self.logger.error(f"⚠️ [后台] 缓存清理任务异常: {e}")

        cleanup_thread = threading.Thread(target=cleanup_worker, daemon=True)
        cleanup_thread.start()
        self.logger.info(f"✅ 后台缓存清理线程已启动（间隔: {interval_hours}小时）")

    def trigger_cleanup(self):
        """手动触发缓存清理"""
        self.logger.info("🔄 手动触发缓存清理...")
        self.clear_expired_cache()
        return "缓存清理完成"
